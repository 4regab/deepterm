...
describe('practiceQuestions', () => {
  it('shows a visible blank in the fill-in prompt', () => {
    expect(buildFillBlankPrompt(cards[0].definition)).toContain('_____')
    expect(buildFillBlankPrompt(cards[0].definition)).toContain(cards[0].definition)
  })
  ...
  it('builds fill-blank questions with a blank and the term as the answer', () => {
  ...
  it('never asks a false pairing when the deck has only one card', () => {
    const questions = generateQuestionsFromCards(cards.slice(0, 1), ['trueFalse'], 1, () => 0.9)
    expect(questions).toHaveLength(1)
    expect(questions[0]?.tfIsCorrectPairing).toBe(true)
