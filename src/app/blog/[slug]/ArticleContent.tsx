...
function extractContent(raw: string): string {
  ...
  if (text.startsWith('{')) {
    try {
      ...
      // Find "content": "..." and extract the value, handling escaped quotes
      const contentMatch = text.match(/"content"\s*:\s*"/)
      if (contentMatch && contentMatch.index !== undefined) {
        const valueStart = contentMatch.index + contentMatch[0].length
        
        // Find the end of the content value - look for "\n} or just "} at the end
        // The content ends with an unescaped quote followed by newline and }
        let endIndex = text.length - 1
        
        // Search backwards for the closing pattern: "\n}
...
export default function ArticleContent({ content }: ArticleContentProps) {
  ...
  // Remove any citation brackets [1], [2, 3], etc. that AI might still generate
  htmlContent = htmlContent.replace(/\s*\[\d+(?:,\s*\d+)*\]/g, '')
  htmlContent = htmlContent.replace(/\s*\[[\d,\s]+\]/g, '')
